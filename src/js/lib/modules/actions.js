...
/* Запуская метод closest в ДжаваСкрипте мы либо получаем метод closest на котором сработал этот метод, либо выше по иерархии. И
если вдруг мы не найдем этот селектор в родителях выше у элемента, то мы ничего не получим. По этому когда мы будем запускать этот
метод возможно у нас не будет какого-то этога */
    let counter = 0; // Что бы понимать сколько элементов мы нашли при помощи closest

    for(let i = 0; i < this.length; i++) {
// Внутри каждого элемента будем находить с помощью closest нужное нам значение и записывать его в this[i]
        this[i] = this[i].closest(selector);
        counter++;
    }
    /* Нужно решить баг что если вдруг здесь ничего не будет найдено this[i].closest(selector) */
...
/* Создаем метод который будет получать все соседние элементы НЕ ВКЛЮЧАЯ САМ ЭЛЕМЕНТ(условно у нас есть 5 блоков, 
мне нужно получить 4 кроме того одного над которым я что-то сделал). Мы это часто делаем когда нужно убрать класс активности у всех
табов просле какого-то одного после клика пользователя на него */
$.prototype.siblings = function() {
    ...
    for(let i = 0; i < copyObj.length; i++) {
        const arr = copyObj[i].parentNode.children; /* parentNode = Обращаемся к родителю блока, например сейчас родитель это findme
 children = получаем детей*/

        // Сейчас в this мы записываем всех соседей, но нам нужно исключить тот блок на котором сработал этот метод
        for(let j = 0; j < arr.length; j++) {

        if (copyObj[i] === arr[j]) { // Если этот элемент будет равен тому элементу что сейчас перебирается в arr[j] (элемент на котором было действие)
            continue; // то мы запускаем континью, мы его не добавляем в главный объект, таким образом он исключится из выборки
        } 
        ...
        numberOfItems += arr.length - 1; /* ТАК КАК МЫ УБРАЛИ ОДИН ЭЛЕМЕНТ С ПОМОЩЬЮ if (copyObj[i] === arr[j]) ТО МЫ СЮДА ПИШЕМ -1
ПОТОМУ ЧТО ПОТОМ МЫ ЗАПИСЫВАЕМ ЭТУ ПЕРЕМЕННУЮ В this.length, И ЗНАЧЕНИЕ ДОЛЖНО БЫТЬ НА ОДИН ЭЛЕМЕНТ МЕНЬШЕ  */
    }

    this.length = numberOfItems;

// Если кство соседем было меньше чем то кство элементов которое было до этого в нашем объекте, то мы просто удалим от туда
    const objLength = Object.keys(this).length;
    for (; numberOfItems < objLength; numberOfItems++) {
